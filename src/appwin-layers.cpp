...
void AppWin::moveZLayersDown()
{
  ...
  if (!pw->zLayerSel(1)) { 
    ...
    pw->repaint();   
    actZLayDown->setEnabled(!pw->zLayerSel(1));
    actZLayUp->setEnabled(!pw->zLayerSel(pw->NumZLayers));
  }
}
...
void AppWin::selectZLayers()
{
  ...
  for (i=pw->NumZLayers-1; i>=0; i--) {
    ...
    dialog->zLayers->setItemSelected(dialog->zLayers->item(ii), selFlag);
    ii++;
  }
  ...
  if (res==QDialog::Accepted) {
    ...
    for (iplt=istart; iplt<=iend; iplt++) {
      ...
      ii = 0;
      for (i=pw->NumZLayers-1; i>=0; i--) {
	selFlag = dialog->zLayers->isItemSelected(dialog->zLayers->item(ii));
