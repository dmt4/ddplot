...
void CalcDPosBurgCircuit(PltWin *pw, bool showdlg, double &xdinit, double &ydinit, double &xdrel, double &ydrel)
{
  QPainter *p;
  QBrush *brush;
  QPen *pen;
  ...
  double x1, y1, x2, y2, Blen, Blen1, Blen2, xmnrel, xmxrel, ymnrel, ymxrel, xdrel_tol, ydrel_tol;
  double xmninit, xmxinit, ymninit, ymxinit, xdinit_tol, ydinit_tol, r1, r2;
  IVector at1, at2, idx1init, idx1rel, idx2init, idx2rel, idx01init, idx01rel, idx02init, idx02rel, aidx;
  int i, ii, j, k, l, n, a, aprev, iV1init, iV1rel, iV2init, iV2rel, iV01rel, iV02rel, a1, a2;
  int iV0init, iV0rel, iVinit, iVrel, nr, i1, i2;
  bool xfound_init, xfound_rel, firstcut, ok, candecide;

  if (pw->interactive)
    pw->setAttribute(Qt::WA_PaintOutsidePaintEvent);
  ...
  for (i1=1; i1<=pw->NRel; i1++) {            // pw->NRel = pw->NInit
	...
	if (idx01rel(k)>0) 
      ...
      candecide = BVectFromCircuit(pw, iV01rel, at1, du, B1, Blen1);
      du.Free();
	...
	if (idx02rel(k)>0) 
      ...
      candecide = BVectFromCircuit(pw, iV02rel, at2, du, B2, Blen2);
      du.Free();
      if (!candecide) continue;
  ...
  for (i=1; i<=iVrel; i++) {
    ...
    pw->dpoly(i,1) = Vrel(i,1);
    pw->dpoly(i,2) = Vrel(i,2);
  }
  xdrel /= iVrel-1;
  ydrel /= iVrel-1;
  ...
  if (showdlg) {
    ...
    for (i=1; i<iVinit; i++) {
      txt.sprintf("%0.4lf", Vinit(i,1));
      QTableWidgetItem *witem1 = new QTableWidgetItem(txt);
      ui.coorNodes_init->setItem(i-1, 0, witem1);
      txt.sprintf("%0.4lf", Vinit(i,2));
      QTableWidgetItem *witem2 = new QTableWidgetItem(txt);
    ...
    txt.sprintf("%0.4lf", Bv(1));  
    ui.Bx->setText(txt); 
    txt.sprintf("%0.4lf", Bv(2));  
    ui.By->setText(txt); 
    txt.sprintf("%0.4lf", Bv(3));  
    ui.Bz->setText(txt); 
