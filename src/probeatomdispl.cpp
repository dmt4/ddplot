...
void ProbeAtomDispl::printFileView()
{
  ...
  for (i=0; i<pw->napicked; i++) {
    ...
    if (chkAtomPosInit->isChecked()) {
      part.sprintf("%8.4f ", posInit[i]);
      txt.append(part);
    }
    
    if (chkAtomPosRel->isChecked()) {
      part.sprintf("%8.4f ", posRel[i]);
      txt.append(part);
    }

    if (chkxyzInit->isChecked()) {
      part.sprintf("%8.4f %8.4f %8.4f ",
    ...
    if (chkRelDispZ->isChecked()) {
      part.sprintf("%8.4f ", dUZ[i]);
      txt.append(part);
    }

    if (chkScrew->isChecked()) {
      part.sprintf("%8.4f ", ddScrew[i]);
      txt.append(part);
    }
    
    if (chkEdge->isChecked()) {
      part.sprintf("%8.4f", ddEdge_mag[i]);
      txt.append(part);
    }
